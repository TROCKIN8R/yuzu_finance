    ...
    <div className="ui-card p-4 flex flex-col gap-3">
      <div className="flex items-start justify-between gap-2">
        <div>
          ...
          <p className="text-xs text-muted mt-0.5">90 prochains jours · brouillon pour révision</p>
        </div>
      ...
      {!loading && rows.length > 0 && (
        <ul className="divide-y divide-border">
          {rows.map((r) => (
            <li key={r.id} className="py-2.5 first:pt-0 last:pb-0 flex items-start justify-between gap-3">
              ...
              <span className={`text-xs font-medium shrink-0 ${toneClass(r.due_date)}`}>
                {dueLabel(r.due_date)}
              </span>
            </li>
          ))}

...
export function TimePage() {
  ...
  async function load() {
    const [p, entries, emp] = await Promise.all([
      ...
      supabase
        .from('time_entries')
        .select('*, projects(name, default_hourly_rate, billing_type, fixed_price, partner_id, partners(legal_name)), employees(first_name, last_name), invoices(invoice_number)')
  ...
  async function save(e: React.FormEvent) {
    ...
    const payload = {
      ...
      billable: internalFixed ? false : form.billable,
    ...
    <PageShell>
      {embedded ? (
        <StepPanelHeader
          ...
          totalSteps={4}
          title="Temps"
          hint="Heures par projet — horaire (facturable) ou forfait (suivi interne, non visible client)."
          actions={
        ...
        <PageHeader
          ...
          subtitle="Horaire (facturable) et forfait (suivi interne, non visible client)."
          actions={
            <Button onClick={openNew} disabled={projects.length === 0 || employees.length === 0}>
              Logger du temps
            </Button>
        ...
        <DataTable minWidth={800}>
            ...
            <tbody className="divide-y divide-border">
              {filtered.map((t) => {
                ...
                const amt = t.billable && proj && !internal ? lineAmount(Number(t.hours), rate) : 0
                const inv = relationOne((t as TimeEntry & { invoices?: { invoice_number: string } | { invoice_number: string }[] }).invoices)
                const emp = relationOne(t.employees)
                return (
                  <tr key={t.id} className="hover:bg-stone-50/50">
                    ...
                    <td className="px-4 py-3">{internal ? '—' : t.billable ? formatCad(amt) : '—'}</td>
                    <td className="px-4 py-3">
                      {internal ? (
        ...
        <form onSubmit={save} className="space-y-3">
            ...
            <select
              ...
              onChange={(e) => {
                ...
                setForm({
                  ...
                  billable: project && isFixedProject(project) ? false : form.billable,
                })
              }}
            >
              {projects.map((p) => (
          ...
          {fixedInternal && (
            <p className="text-xs text-muted -mt-1">
              Projet forfaitaire — ce temps est pour le reporting interne et n&apos;apparaît pas sur la facture client.
            </p>
          )}

...
-- ---------------------------------------------------------------------------
-- Project week plans (pipeline — expected hours per calendar week)
-- ---------------------------------------------------------------------------

create table public.project_week_plans (
  id uuid primary key default gen_random_uuid(),
  user_id uuid not null references auth.users (id) on delete cascade,
...
create index project_week_plans_user_id_idx on public.project_week_plans (user_id);
create index project_week_plans_project_id_idx on public.project_week_plans (project_id);
create index project_week_plans_week_start_idx on public.project_week_plans (user_id, week_start);
...
create unique index time_entries_project_day_uniq on public.time_entries (
  user_id,
  project_id,
  entry_date,
...
-- ---------------------------------------------------------------------------
-- Document attachments (invoices, supplier bills, employee receipts)
...
create index document_attachments_user_id_idx on public.document_attachments (user_id);
create index document_attachments_entity_idx on public.document_attachments (entity_type, entity_id);
